...
> Location within the framework `Clarisse-agent-framework/emitter`.
...
```ts
import { Emitter, EventMeta } from "Clarisse-agent-framework/emitter/emitter";
...
```ts
import { Emitter, EventMeta, Callback } from "Clarisse-agent-framework/emitter/emitter";
...
await emitter.emit("update", { id: 123, data: "Hello Clarisse!" });
```
...
```ts
import { Callback, Emitter } from "Clarisse-agent-framework/emitter/emitter";
import { BaseLLM } from "Clarisse-agent-framework/llms/base";
...
import { UnconstrainedMemory } from "Clarisse-agent-framework/memory/unconstrainedMemory";
...
> [!TIP]
>
> The more complex agentic example can be found [here](/examples/agents/Clarisse.ts).
