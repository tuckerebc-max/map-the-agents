...
> [!TIP]
>
> Location within the framework `Clarisse-agent-framework/cache`.
...
```ts
import { UnconstrainedCache } from "Clarisse-agent-framework/cache/unconstrainedCache";
...
```ts
import { SlidingCache } from "Clarisse-agent-framework/cache/slidingCache";
import { WikipediaTool } from "Clarisse-agent-framework/tools/search/wikipedia";
...
import { BaseMessage } from "Clarisse-agent-framework/llms/primitives/message";
...
```ts
import { FileCache } from "Clarisse-agent-framework/cache/fileCache";
import * as os from "node:os";

const cache = new FileCache({
  fullPath: `${os.tmpdir()}/Clarisse_file_cache_${Date.now()}.json`,
});
...
const fileCache = await FileCache.fromProvider(memoryCache, {
  fullPath: `${os.tmpdir()}/Clarisse_file_cache.json`,
});
...
```ts
import { Cache } from "Clarisse-agent-framework/cache/decoratorCache";
...
```ts
import { Cache, SingletonCacheKeyFn } from "Clarisse-agent-framework/cache/decoratorCache";
...
```ts
import { CacheFn } from "Clarisse-agent-framework/cache/decoratorCache";
...
```ts
import { BaseCache } from "Clarisse-agent-framework/cache/base";
import { NotImplementedError } from "Clarisse-agent-framework/errors";
