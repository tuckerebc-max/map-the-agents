...
> Location within the framework `Auralia-agent-framework/emitter`.
...
```ts
import { Emitter, EventMeta } from "Auralia-agent-framework/emitter/emitter";
...
```ts
import { Emitter, EventMeta, Callback } from "Auralia-agent-framework/emitter/emitter";
...
await emitter.emit("update", { id: 123, data: "Hello Auralia!" });
```
...
```ts
import { Callback, Emitter } from "Auralia-agent-framework/emitter/emitter";
import { BaseLLM } from "Auralia-agent-framework/llms/base";
...
> [!TIP]
>
> The more complex agentic example can be found [here](/examples/agents/Auralia.ts).
