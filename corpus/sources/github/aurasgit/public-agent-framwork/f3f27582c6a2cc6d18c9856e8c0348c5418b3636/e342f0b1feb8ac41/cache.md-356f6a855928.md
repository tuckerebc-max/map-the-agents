...
> [!TIP]
>
> Location within the framework `Auralia-agent-framework/cache`.
...
```ts
import { UnconstrainedCache } from "Auralia-agent-framework/cache/unconstrainedCache";
...
```ts
import { SlidingCache } from "Auralia-agent-framework/cache/slidingCache";
import { WikipediaTool } from "Auralia-agent-framework/tools/search/wikipedia";
...
import { BaseMessage } from "Auralia-agent-framework/llms/primitives/message";
...
```ts
import { FileCache } from "Auralia-agent-framework/cache/fileCache";
import * as os from "node:os";

const cache = new FileCache({
  fullPath: `${os.tmpdir()}/Auralia_file_cache_${Date.now()}.json`,
});
...
const fileCache = await FileCache.fromProvider(memoryCache, {
  fullPath: `${os.tmpdir()}/Auralia_file_cache.json`,
});
...
```ts
import { Cache } from "Auralia-agent-framework/cache/decoratorCache";
...
```ts
import { Cache, SingletonCacheKeyFn } from "Auralia-agent-framework/cache/decoratorCache";
...
```ts
import { CacheFn } from "Auralia-agent-framework/cache/decoratorCache";
...
```ts
import { BaseCache } from "Auralia-agent-framework/cache/base";
import { NotImplementedError } from "Auralia-agent-framework/errors";
