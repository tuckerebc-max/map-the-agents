# Contributing to Qwen CLI (Community Fork)

We welcome contributions to this community fork of Google's Gemini CLI!

## About This Fork

This is a **community-maintained fork** of [Google's Gemini CLI](https://github.com/google-gemini/gemini-cli), modified to work with Qwen models from Alibaba Cloud. This project is:

- **Independent**: Not affiliated with Google or Alibaba Cloud
- **Community-driven**: Maintained by contributors like you
- **Open Source**: Licensed under Apache 2.0 (same as original)

## Contributing Guidelines

### Before you begin

Since this is a community fork:

- **No CLA required**: Unlike the original Google project, we don't require a Contributor License Agreement
- **Community standards**: We follow standard open-source community guidelines
- **Attribution**: We properly credit the original Google team and maintain transparency about this being a fork

### Code of Conduct

We follow a welcoming and inclusive code of conduct. Be respectful, constructive, and helpful to all community members.

## Areas We Welcome Contributions

### High Priority
- **Qwen Model Integration**: Improvements to Qwen API integration and model support
- **Bug Fixes**: Fixing issues specific to Qwen models or the fork
- **Documentation**: Improving docs for this community fork
- **Testing**: Adding tests for Qwen-specific functionality

### Welcome Contributions  
- **Feature Parity**: Keeping up with useful features from the original Gemini CLI
- **Performance**: Optimizations for Qwen model usage
- **Examples**: Adding examples showing Qwen capabilities
- **Tooling**: Improving development and build processes

### Please Coordinate First
- **Major Architecture Changes**: Discuss in issues before implementing
- **Breaking Changes**: We try to maintain compatibility when possible
...
2. **Clone your fork**:
   ```bash
   git clone https://github.com/[your-username]/qwen-cli-fork.git
   cd qwen-cli-fork
   ```
...
4. **Build the project**:
   ...
   npm run bundle
   ```
5. **Set up your Qwen API key** for testing:
   ```bash
...
All submissions require review through GitHub pull requests. We
...
If you are contributing to this fork, you can run the built, test and integration test workflows. However, to make the integration tests run you'll need to add a [GitHub Repository Secret](https://docs.github.com/en/actions/security-for-github-actions/security-guides/using-secrets-in-github-actions#creating-secrets-for-a-repository) with a value of `DASHSCOPE_API_KEY` and set that to a valid Qwen API key. Your key and secret are private to your repo; no one without access can see your key.
...
Container-based [sandboxing](#sandboxing) is highly recommended and requires, at a minimum, setting `QWEN_SANDBOX=true` in your `~/.env` and ensuring a container engine (e.g. `docker` or `podman`) is available. See [Sandboxing](#sandboxing) for details.
...
To start the Qwen CLI from the source code (after building), run the following command from the root directory:

```bash
npm start
```

If you’d like to run the source build outside of the qwen-cli-fork folder you can utilize `npm link path/to/qwen-cli-fork/packages/cli` (see: [docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) or `alias qwen="node path/to/qwen-cli-fork/packages/cli"` to run with `gemini`
