# yt-channels-DS-AI-ML-CS (`yt-channels-ds-ai-ml-cs`)

[Back to directory index](../index.md)

Directory membership: backing-only.

- Category: other
- Provider/maker: benthecoder
- License: none indicated
- Language: Markdown
- Interface: install=web
- Model providers: None — curated resource list, no AI models
- Feature flags (directory-reported):
  - mcp_support: n/a (reported)
  - plugin_support: n/a (reported)
  - claude_code_plugin: n/a (reported)
  - subagents: n/a (reported)
  - hooks: n/a (reported)
  - plan_mode: n/a (reported)

Repository map entry: [benthecoder/yt-channels-ds-ai-ml-cs](../../repos/benthecoder/yt-channels-ds-ai-ml-cs.md) (source: backing, field: `source_code_url`).

## Description

(backing feed `description`, not a verified repo-code finding)
yt-channels-DS-AI-ML-CS is a curated awesome list of more than 180 YouTube channels covering data science, machine learning and deep learning, data engineering, statistics, AI research, computer scien
Sources: [backing feed @ 8664d24144c7](https://github.com/prime-radiant-inc/alltheagents.org/blob/8664d24144c79ac7400a6799c00f070897d856a6/_data/agents.json)
