# Ai-Agent-Playground (`ai-agent-playground`)

[Back to directory index](../index.md)

Directory membership: backing-only.

- Category: other
- Provider/maker: Wei1024
- License: unknown
- Language: Python
- Interface: install=git clone + pip install -r requirements.txt (Python application with app.py entry point)
- Model providers: unknown
- Feature flags (directory-reported):
  - mcp_support: no (no)
  - plugin_support: no (no)
  - claude_code_plugin: no (no)
  - subagents: no (no)
  - hooks: no (no)
  - plan_mode: no (no)

Repository map entry: [wei1024/ai-agent-playground](../../repos/wei1024/ai-agent-playground.md) (source: backing, field: `source_code_url`).

## Description

(backing feed `description`, not a verified repo-code finding)
The repository serves as its author's test bed for building AI agents aimed at office workers. The implemented stack is deliberately small: an AutoGen agent that follows a ReAct thought process, a Tav
Sources: [backing feed @ 8664d24144c7](https://github.com/prime-radiant-inc/alltheagents.org/blob/8664d24144c79ac7400a6799c00f070897d856a6/_data/agents.json)
