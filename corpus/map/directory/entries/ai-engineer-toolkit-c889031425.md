# ai-engineer-toolkit (`ai-engineer-toolkit`)

[Back to directory index](../index.md)

Directory membership: backing-only.

- Category: other
- Provider/maker: break-into-data
- License: unknown
- Language: TypeScript
- Interface: unknown
- Model providers: OpenAI, Google Gemini, Anthropic, Hugging Face, xAI Grok
- Feature flags (directory-reported):
  - mcp_support: no (no)
  - plugin_support: no (no)
  - claude_code_plugin: no (no)
  - subagents: no (no)
  - hooks: no (no)
  - plan_mode: no (no)

Repository map entry: [break-into-data/ai-engineer-toolkit](../../repos/break-into-data/ai-engineer-toolkit.md) (source: backing, field: `source_code_url`).

## Description

(backing feed `description`, not a verified repo-code finding)
The AI engineering tool landscape changes faster than most engineers can track, and choosing a stack means cross-referencing pricing, maturity, and docs across dozens of vendors. This repository organ
Sources: [backing feed @ 0861b8ee1c27](https://github.com/prime-radiant-inc/alltheagents.org/blob/0861b8ee1c271047d55caa72efdfc3a6d2046174/_data/agents.json)
